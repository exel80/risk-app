...
export default function LogTable({ data }) {
    return (
        <>
            ...
            {data && <p>Defender lost {data.matches[data.matches.length - 1].defender.current - data.matches[0].defender.total} troop(s)</p>}
            {data && <p>Attacker lost {data.matches[data.matches.length - 1].attacker.current - data.matches[0].attacker.total} troop(s)</p>}
        </>
    )
}